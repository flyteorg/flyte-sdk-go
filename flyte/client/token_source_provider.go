...
func NewTokenSourceProvider(ctx context.Context, cfg *Config, tokenCache cache.TokenCache,
	authClient authconnect.AuthMetadataServiceClient) (TokenSourceProvider, error) {
	...
	case AuthTypeClientSecret:
		...
		if len(tokenURL) == 0 {
			metadata, err := authClient.GetOAuth2Metadata(ctx, connect.NewRequest(&auth.GetOAuth2MetadataRequest{}))
			if err != nil {
				return nil, fmt.Errorf("failed to fetch auth metadata. Error: %v", err)
			}

			tokenURL = metadata.Msg.GetTokenEndpoint()
		}
		...
		if len(scopes) == 0 || cfg.UseAudienceFromAdmin {
			publicClientConfig, err := authClient.GetPublicClientConfig(ctx, connect.NewRequest(&auth.GetPublicClientConfigRequest{}))
			if err != nil {
				return nil, fmt.Errorf("failed to fetch client metadata. Error: %v", err)
			}
			// Update scopes from publicClientConfig
			if len(scopes) == 0 {
				scopes = publicClientConfig.Msg.GetScopes()
			}
			// Update audience from publicClientConfig
			if cfg.UseAudienceFromAdmin {
				audienceValue = publicClientConfig.Msg.GetAudience()
			}
		}
